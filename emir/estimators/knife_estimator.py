...
class KNIFEEstimator:
    ...
    def eval(
        ...
        self.knife = KNIFE(
            self.args, self.x_dim, self.y_dim, kernel_type=kernel_type
        ).to(self.args.device)
    ...
    def fit_estimator(self, x, y, record_loss: Optional[bool] = False) -> List[float]:
        ...
        train_loader = FastTensorDataLoader(x, y, batch_size=self.args.batch_size,
        )

        for epoch in trange(
            self.args.n_epochs, position=-1, desc="Knife training", leave=False
        ):
            epoch_loss = []
