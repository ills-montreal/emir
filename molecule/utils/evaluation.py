...
def get_dataloaders(
    ...
    dataset_train = torch.utils.data.TensorDataset(
        split_emb["train"]["x"].float(), split_emb["train"]["y"].to(device)
    )
    dataset_val = torch.utils.data.TensorDataset(
        split_emb["valid"]["x"].float(), split_emb["valid"]["y"].to(device)
    )
    dataset_test = torch.utils.data.TensorDataset(
        split_emb["test"]["x"].float(), split_emb["test"]["y"].to(device)
    )
...
class Feed_forward(torch.nn.Module):
    def __init__(self, input_dim, output_dim, config: FFConfig):
        ...
        if self.config.n_layers == 0:
            ...
            self.inp_layer = torch.nn.Sequential(
                torch.nn.Linear(input_dim, self.config.hidden_dim, device=self.config.device),
            ...
            if self.config.n_layers > 1:
                ...
                for i in range(self.config.n_layers):
                    self.hidden_layers.append(
                        torch.nn.Linear(self.config.hidden_dim, self.config.hidden_dim, device=self.config.device)
                    )
                    self.hidden_layers.append(torch.nn.Dropout(self.config.d_rate))
                    self.hidden_layers.append(torch.nn.ReLU())
                    self.hidden_layers.append(self.norm_fn(self.config.hidden_dim, device=self.config.device))
    ...
    def forward(self, x):
        if self.config.n_layers == 0:
            return self.inp_layer(x)
        if self.config.n_layers == 1:
            return self.out_layer(self.inp_layer(x))
    ...
    def evaluate(self, dataloader_val: DataLoader, record: bool = True):
        ...
        self.val_acc.append(((y_hat_val > 0.5).float() == y_true).float().mean().item())
        self.val_aucpr.append(
        ...
        self.val_f1.append(
            f1_score(
                ...
                (y_hat_val.detach().cpu().numpy() > 0.5).astype(int),
            )
        )
        self.val_roc.append(
            roc_auc_score(
    ...
    def plot_loss(self, title=""):
        ...
        axes[2].plot(self.val_aucpr, label="val")
        axes[2].set_title("AUCPR evolution over the training")
