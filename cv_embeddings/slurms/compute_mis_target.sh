...
output_dir=$1
embeddings_dir=$2 # path to embeddings dir, should contain hf_model_dir/hf_model_name/hf_dataset_dir/hf_dataset_name/embeddings.npy
log_dir=output_dir/logs
n_modes=$4 # 8


cond_models="izhx/udever-bloom-560  sentence-transformers/all-MiniLM-L6-v2"

# Compute for all Y : I(X -> Y) = H(Y) - H(Y|X)
# We compute H(Y) only once and then compute H(Y|X) for all X (in cond_models)

for model_Y in  "izhx/udever-bloom-560"  "sentence-transformers/all-MiniLM-L6-v2"; do
