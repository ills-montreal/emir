...
def parse_args():
    ...
    parser.add_argument("--bart_path", type=Path, default="BARTScore/bart_score.pth")
...
def evaluate_bartbert(df, device="cuda", bart_path="BARTScore/bart_score.pth"):
    ...
    scorer.load(path=bart_path)
    metrics = {'BARTScore': []}

    scores = scorer.score(srcs=texts, tgts=summaries, batch_size=8)

    metrics['BARTScore'] = scores
...
def main():
    ...
    metrics = evaluate_bartbert(df, device=args.device, bart_path=args.bart_path)
