...
def make_table_model_summary(df):
    ...
    cols = ['metadata/#params', 'common/rougeLsum', 'common/BERTScore', 'common/BARTScore'] + SHM + ['I(summary -> text)', 'H(text|summary)', 'H(summary|text)']
    ...
    avg = avg.rename(columns={'I(summary -> text)': '$I(T,S)$', 'Model name': 'Model', 'Dataset name': 'Dataset',
                              'Main ideas': "M. I.", "Attribution": "Attr.", "Repetition": "Rep.",
                              "Comprehensible": "Compr.", "Conciseness": "Conc.", "Grammar": "Gram.",
                              "H(text|summary)": "H(T|S)", "H(summary|text)": "H(S|T)"
                              })
    ...
    avg = avg[['Size', '\\texttt{ROUGE-L}', '\\texttt{BERTScore}', "\\texttt{BARTScore}", 'M. I.', 'Attr.', 'Rep.', 'Compr.', 'Conc.', 'Gram.', '$I(T,S)$',
               'H(T|S)', 'H(S|T)']]
...
def make_correlation_table(df):
    ...
    for dataset in datasets:
        ...
        df_dataset = df_dataset[ROUGES + SHM + MI + classification_tasks + embedding_tasks].corr("spearman")
        # add dataset name
    ...
    df_corr = pd.concat(
        {'SH.': df_corr[[c for c in df_corr.columns if "SHMetric" in c]], 'CT.': df_corr[classification_tasks],
         'Emb.': df_corr[embedding_tasks], "Common": df_corr[ROUGES]}, axis=1)

    idx = pd.IndexSlice

    # Select index to be displayed
    df_corr = df_corr.loc[idx[:, ['I(summary -> text)',
                                  ] + ROUGES + SHM_interesting], :]
